...
//} Regex for matching port names we are interested in
var INTERESTING_PORTS = /usb|acm|com/i;
...
PololuMaestro.find = function(mode, onReady) {
	...
	SerialPort.list(function (error, result) {
		var ports = result.filter(function(port) {
			// Match only ports that we could be connected to
			// ttyUSB#, cu.usbmodem#, COM#
			if(!INTERESTING_PORTS.test(port.comName)) {
				return false;
			}
